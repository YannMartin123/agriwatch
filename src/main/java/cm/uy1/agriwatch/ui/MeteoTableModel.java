...
public class MeteoTableModel extends AbstractTableModel {
    ...
    // Seuil d'alerte configurable dynamiquement depuis l'IHM (Settings)
    public static volatile double seuilAlerte = 30.0;
     ...
     * Appelle fireTableRowsUpdated/Inserted pour rafraîchir la JTable.
     */
    ...
    /**
     * Nombre de zones actuellement en alerte (humidité < seuilAlerte).
     */
    public int getNombreAlertes() {
        synchronized (donnees) {
            int count = 0;
            for (MesureMeteo m : donnees) {
                if (m.getHumidite() < seuilAlerte) {
                    count++;
                }
            }
            return count;
    ...
    public boolean estEnAlerte(Zone zone) {
        synchronized (donnees) {
            for (MesureMeteo m : donnees) {
                if (m.getZone() == zone && m.getHumidite() < seuilAlerte) {
                    return true;
                }
            }
            return false;
    ...
    public Object getValueAt(int row, int col) {
        synchronized (donnees) {
            if (row >= donnees.size()) return null;
            ...
            return switch (col) {
                ...
                case 3  -> m.getHumidite() < seuilAlerte ? "⚠ ALERTE" : "Normal";
                default -> null;
            };
        }
    }
